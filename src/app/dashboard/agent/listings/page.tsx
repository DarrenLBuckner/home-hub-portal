...
function ListingsContent() {
  ...
  const TAB_CONFIG: { key: FilterTab; label: string }[] = [
    ...
    { key: 'sale', label: tProps('forSale') },
    { key: 'rent', label: tProps('forRent') },
    { key: 'lease', label: tProps('forRent') },
    { key: 'under_contract', label: tProps('underContract') },
    { key: 'sold', label: tProps('sold') },
    { key: 'rented', label: tProps('rented') },
    { key: 'pending', label: tProps('pending') },
    { key: 'draft', label: tProps('draft') },
  ];
        ...
        <div className="print-record hidden" style={{ display: 'none' }}>
          <div style={{ fontFamily: 'Arial, sans-serif', fontSize: '11pt', maxWidth: '700px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '18pt', borderBottom: '2px solid #059669', paddingBottom: '8px', marginBottom: '4px' }}>
              {siteName} &mdash; Property Record
            </h1>
            ...
            <div style={{ marginTop: '32px', borderTop: '1px solid #ccc', paddingTop: '8px', fontSize: '9pt', color: '#999' }}>
              Property ID: {printProperty.id} | Generated by {siteName} | portalhomehub.com
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 no-print" data-print="hide">
        ...
        <div className="mb-4">
          <Link href="/dashboard/agent" className="text-emerald-600 hover:underline text-sm">
            &larr; {tCommon('back')} {tDashboard('overview')}
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">{tDashboard('myListings')}</h1>
            <p className="text-sm text-gray-500">{tProps('title')}</p>
          </div>
          <Link
        ...
        {filtered.length === 0 && (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            <p className="text-gray-500 mb-4">{tProps('noListings')}</p>
            <Link
              href="/dashboard/agent/create-property"
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <th className="px-4 py-3">{tProps('title')}</th>
                    <th className="px-4 py-3">Status</th>
                    <th className="px-4 py-3">Type</th>
                    <th className="px-4 py-3">{tDetails('price')}</th>
                    <th className="px-4 py-3">{tDetails('bedrooms')}/{tDetails('bathrooms')}</th>
                    <th className="px-4 py-3">{tDetails('location')}</th>
                    <th className="px-4 py-3">{tListing('listedOn')}</th>
                    <th className="px-4 py-3">{tListing('viewCount')}</th>
                    <th className="px-4 py-3">{tCommon('edit')}</th>
                  </tr>
                </thead>
