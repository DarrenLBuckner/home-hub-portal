...
interface CountryCardProps {
  ...
  signupFlagsByCountry: Record<string, TerritorySignupFlags>;
}

function CountryCard({ country, selectedCountry, onCountrySelect, signupFlagsByCountry }: CountryCardProps) {
  ...
  // Per-territory gating. Fail closed: a country absent from the map (lookup
  // failure) hides both owner-type buttons. Agent button is never gated here.
  const flags = signupFlagsByCountry[country.code];
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="space-y-3">
          ...
          {(flags?.landlordSignupEnabled || flags?.fsboSignupEnabled) && (
            <div className="grid grid-cols-2 gap-3">
              {flags?.landlordSignupEnabled && (
                <button
...
function SelectCountryContent({ signupFlagsByCountry }: { signupFlagsByCountry: Record<string, TerritorySignupFlags> }) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {countries.filter(country => country.region === 'caribbean' && country.active).map((country) => (
              <CountryCard
                ...
                signupFlagsByCountry={signupFlagsByCountry}
              />
            ))}
          </div>
        </div>
              ...
              <div className="p-6">
                ...
                {country.active ? (
                  <div className="space-y-3">
                    ...
                    {(signupFlagsByCountry[country.code]?.landlordSignupEnabled ||
                      signupFlagsByCountry[country.code]?.fsboSignupEnabled) && (
                      <div className="grid grid-cols-2 gap-3">
                        {signupFlagsByCountry[country.code]?.landlordSignupEnabled && (
                          <button
...
export default function SelectCountryClient({
  signupFlagsByCountry,
}: {
  signupFlagsByCountry: Record<string, TerritorySignupFlags>;
}) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Loading...</div>}>
      <SelectCountryContent signupFlagsByCountry={signupFlagsByCountry} />
    </Suspense>
  );
}