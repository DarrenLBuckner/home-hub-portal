...
const AITitleSuggester: React.FC<AITitleSuggesterProps> = ({
  ...
  const generateTitles = async () => {
    if (!propertyData.propertyType) {
      setError('Please select a property type first (Step 1)');
      return;
    }

    const locationContext = propertyData.neighborhood || propertyData.location;
    if (!locationContext?.trim()) {
      setError('Please enter a neighborhood above for better titles');
      return;
    }
  ...
  // Readiness indicator
  const getReadiness = () => {
    if (!propertyData.propertyType) return { ready: false, text: 'Select property type first', icon: '⚠️' };
    if (!locationContext?.trim()) return { ready: false, text: 'Enter neighborhood above', icon: '⚠️' };
    return { ready: true, text: 'Ready to generate titles', icon: '✓' };
  };
  const readiness = getReadiness();

  return (
    <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-3 mt-3">
      {/* Compact header */}
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className="text-lg">✨</span>
          <span className="text-sm font-semibold text-gray-800">AI Title Suggestions</span>
          <span className={`text-xs px-2 py-0.5 rounded-full ${
            readiness.ready ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
          }`}>
            {readiness.icon} {readiness.ready ? 'Ready' : 'Not ready'}
          </span>
        </div>
